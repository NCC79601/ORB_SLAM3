...
void System::Shutdown()
{
    ...
    if(!mStrSaveAtlasToFile.empty())
    {
        ...
        // fix: trying to add TEXT_TYPE atlas saving
        // SaveAltas(FileType::TEXT_FILE);
    }
...
void System::SavePointCloud(const string &filename){
    ...
    // output the vpMPs.size()
    cout << "Number of map points in vpMPs: " << vpMPs.size() << endl;

    int skipped_map_points = 0;
    ...
    // output the number of skipped map points
    cout << "Number of skipped map points: " << skipped_map_points << endl;

    // Close the output stream
    f.close();

    // Get the output file stream in fixed-point format for reference map points
    // fix: use regex to get the reference map filename
    string ref_filename;
    {
        std::regex re("(.*\\/)([^\\/]+)");
        std::smatch match;
        if (std::regex_match(filename, match, re)) {
            ...
            ref_filename = path + "ref_" + name;
        } else {
            // Handle the case where the input string does not match the expected pattern
            ref_filename = filename;
        }
    }
    f.open(ref_filename.c_str());
    f << "pos_x, pos_y, pos_z" << endl;
    f << fixed;

    // output the vpRefMPs.size()
    cout << "Number of reference map points in vpRefMPs: " << vpRefMPs.size() << endl;

    skipped_map_points = 0;
    ...
    // output the number of skipped map points
    cout << "Number of skipped reference map points: " << skipped_map_points << endl;
